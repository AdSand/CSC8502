...
Mesh* Mesh::GenerateQuad()
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		m->normals[i] = Vector3(0.0f, 0.0f, -1.0f); // new - tutorial 13
		m->tangents[i] = Vector4(1.0f, 0.0f, 0.0f, 1.0f); // new - tutorial 13
	}
	m->BufferData();
	return m;
}
...
{
	// to loop around every vertex, and generate the tangent for it,
	// based on the texture coordinates of the triangle it is part of. 
	// As we need texture coordinates for this calculation, if there aren’t any, the function should just return.
	// We account for meshes that modify their data, by allocating the memory for the tangents if necessary,
	// and resetting all tangents to zero
	if (!textureCoords)
	{
		return;
	}
	...
	for (int i = 0; i < triCount; ++i)
	{
		...
		// accomodating index based meshes...
		tangents[a] += tangent;
		tangents[b] += tangent;
		tangents[c] += tangent;
	}
...
{
	// allocate the memory for the normals pointer if necessary
	if (!normals)
	{
...
{
	// The first parameter tells the method which triangle we want to get the vertex locations for.
	// The next three parameters will receive the indices into the vertices array.
	//  Note this method only works for triangle primitives - strips or fans won’t work!
